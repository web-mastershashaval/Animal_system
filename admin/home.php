...
$pets_query = "SELECT p.pet_name, c.owner_name FROM pets p JOIN users c ON p.owner_id = c.user.id";
$pets_result = fetchQueryResult($conn, $pets_query);

$clients_query = "SELECT * FROM users";
    ...
    <div class="overview-section">
        ...
        <ul>
            <?php if ($products_result && $products_result->num_rows > 0): ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <li>No animal products found or failed to load product data.</li>
            <?php endif; ?>
        </ul>
        ...
        <ul>
            <?php if ($pets_result && $pets_result->num_rows > 0): ?>
                <?php while ($row = $pets_result->fetch_assoc()): ?>
                    <li>Pet Name: <?php echo htmlspecialchars($row['pet_name']); ?> - Owner: <?php echo htmlspecialchars($row['firstname']); ?></li>
                <?php endwhile; ?>
            <?php else: ?>
                <li>No pets found or failed to load pet data.</li>
            <?php endif; ?>
        </ul>
        ...
        <ul>
            <?php if ($clients_result && $clients_result->num_rows > 0): ?>
                <?php while ($row = $clients_result->fetch_assoc()): ?>
                    <li>Client: <?php echo htmlspecialchars($row['firstname']); ?> - Contact: <?php echo htmlspecialchars($row['number']); ?></li>
                <?php endwhile; ?>
            <?php else: ?>
                <li>No clients found or failed to load client data.</li>
            <?php endif; ?>
        </ul>
